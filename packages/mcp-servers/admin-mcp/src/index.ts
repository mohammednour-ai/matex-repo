...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    { name: "get_audit_trail", description: "Get recent audit log entries", inputSchema: { type: "object", properties: { limit: { type: "number" }, user_id: { type: "string" }, category: { type: "string" } } } },
  ...
  if (tool === "get_audit_trail") {
    ...
    if (supabase) {
      let query = supabase
        .schema("log_mcp")
        .from("audit_log")
        .select("log_id,server,category,action,user_id,created_at,output_summary")
      ...
      if (error) return fail("DB_ERROR", error.message);
      return { content: [{ type: "text", text: ok({ entries: data ?? [], total: (data ?? []).length }) }] };
    }
