...
export default function AdminPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 border-b border-night-700 pb-3">
        {TABS.map((t) => {
          const Icon = t.icon;
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                active ? "bg-brand-600 text-white shadow-sm" : "bg-night-800 text-night-200 hover:bg-night-700",
              )}
            >
              <Icon size={16} />
              {t.label}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {overview &&
            ["total_users", "total_listings", "total_orders", "open_disputes"].map((k) => (
              <div key={k} className="rounded-xl border border-night-700 bg-night-850 p-4 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-wide text-night-300">{k.replace(/_/g, " ")}</p>
                <p className="text-2xl font-bold text-night-100 mt-1">{String(overview[k] ?? "—")}</p>
              </div>
            ))}
          {!overview && <p className="text-night-300 text-sm">Loading overview…</p>}
        </div>
      )}

      {tab === "users" && (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3 items-end rounded-xl border border-night-700 bg-night-850 p-4">
            <Input
          ...
          <div className="overflow-x-auto rounded-xl border border-night-700 bg-night-850">
            <table className="min-w-full text-sm">
              <thead className="bg-night-900 text-left text-xs font-semibold text-night-200 uppercase">
                <tr>
              ...
              <tbody>
                {users.map((u) => {
                  ...
                  return (
                    <tr key={id} className="border-t border-night-700/60">
                      <td className="px-3 py-2 font-medium text-night-100">{String(u.email ?? "")}</td>
                      <td className="px-3 py-2">
                        <Badge variant={u.account_status === "active" ? "success" : "warning"}>
                          {String(u.account_status ?? "")}
                        </Badge>
                      </td>
                      <td className="px-3 py-2 text-night-200">{String(u.account_type ?? "")}</td>
                      <td className="px-3 py-2 font-mono text-xs text-night-300 max-w-[8rem] truncate" title={id}>
                        {id}
                      </td>
                      <td className="px-3 py-2">
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {listings.map((l) => {
                ...
                return (
                  <tr key={lid} className="border-t border-night-700/60">
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3 items-end rounded-xl border border-night-700 bg-night-850 p-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-night-200">New status</label>
              <select
                value={orderNewStatus}
                onChange={(e) => setOrderNewStatus(e.target.value)}
                className="rounded-lg border border-night-700 px-3 py-2 text-sm"
              >
          ...
          <div className="overflow-x-auto rounded-xl border border-night-700 bg-night-850">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={String(o.order_id)} className="border-t border-night-700/60">
      ...
      {tab === "escrow" && (
        <div className="space-y-4">
          <div className="rounded-xl border border-night-700 bg-night-850 p-4 space-y-3">
            <p className="text-sm font-semibold text-night-100">Escrow actions</p>
          ...
          <div className="space-y-2">
            {escrows.map((e) => (
              <div key={String(e.escrow_id)} className="rounded-lg border border-night-700 bg-night-850 p-3">
                <JsonPreview value={e} />
              </div>
            ))}
          </div>
      ...
      {tab === "auctions" && (
        <div className="space-y-4">
          ...
          <h3 className="text-sm font-bold text-night-100">Auctions</h3>
          <div className="space-y-2">
            {auctions.map((a) => (
              <div key={String(a.auction_id)} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-night-700 bg-night-850 p-3">
                <div>
                  <p className="font-medium text-night-100">{String(a.title ?? "")}</p>
                  <p className="text-xs text-night-300 font-mono">{String(a.auction_id)}</p>
                </div>
          ...
          <h3 className="text-sm font-bold text-night-100">Lots</h3>
          <div className="space-y-2">
            {lots.map((lot) => (
              <div key={String(lot.lot_id)} className="rounded-lg border border-night-700 bg-night-850 p-3">
                <div className="flex flex-wrap justify-between gap-2 mb-2">
                  <span className="text-sm font-mono text-night-200">{String(lot.lot_id)}</span>
      ...
      {tab === "bids" && (
        <div className="space-y-2">
          {bids.map((b) => (
            <div key={String(b.bid_id)} className="rounded-lg border border-night-700 bg-night-850 p-3">
              <JsonPreview value={b} />
            </div>
          ))}
        </div>
      ...
      {tab === "purchases" && (
        <div className="space-y-4">
          <div className="rounded-xl border border-night-700 bg-night-850 p-4 flex flex-wrap gap-3 items-end">
          ...
          <h3 className="text-sm font-bold text-night-100">Recent transactions</h3>
          <div className="space-y-2">
            {txs.map((t) => (
              <div key={String(t.transaction_id)} className="rounded-lg border border-night-700 bg-night-850 p-3">
                <JsonPreview value={t} />
              </div>
            ))}
          </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {configEntries.map((row) => (
                  <tr key={String(row.config_key)} className="border-t border-night-700/60">
                    ...
                    <td className="px-3 py-2 text-night-300 text-xs">{String(row.updated_at ?? "")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === "audit" && (
        <div className="space-y-2">
          {auditEntries.map((row, i) => (
            <div key={String(row.log_id ?? i)} className="rounded-lg border border-night-700 bg-night-850 p-3">
