...
export default function MaterialDetailPage() {
  ...
  if (!material) {
    return (
      <div className="rounded-2xl border border-line bg-surfaceBg p-6 text-center">
        <p className="text-sm font-semibold text-fg-muted">Unknown material</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <Link href="/market" className="inline-flex items-center gap-1 text-xs font-semibold text-fg-muted hover:text-brand-700">
      ...
      {latest?.news_headlines && latest.news_headlines.length > 0 && (
        <section className="rounded-2xl border border-line bg-surfaceBg p-5">
          <div className="flex items-center gap-2">
            <Newspaper className="h-4 w-4 text-fg-subtle" />
            <h3 className="text-sm font-semibold text-fg">Headlines feeding the model</h3>
          </div>
          <ul className="mt-3 space-y-2">
            {latest.news_headlines.map((headline) => (
              <li
                key={headline}
                className="rounded-xl border border-line bg-surfaceBg/90 px-3 py-2 text-xs text-fg-muted"
              >
                {headline}
              </li>
            ))}
